...
@section('content')
<!-- page title -->
    <x-page-title title="Role List" pagetitle="Roles" />
    ...
    <div class="card" id="roleList">
        <div class="card-body">
            <div class="grid grid-cols-1 gap-5 mb-5 xl:grid-cols-2">
                        ...
                        <form action="{{ route('roles.index') }}" method="GET">
                            <div class="flex border border-slate-200 rounded-md overflow-hidden">
                                <input type="text" name="search"
                                       ...
                                       placeholder="Search for roles..." value="{{ $search }}" autocomplete="off">
                ...
                <div class="ltr:md:text-end rtl:md:text-start">
                    ...
                    <button type="button" class="text-white bg-red-500 border-red-500 btn hover:text-white hover:bg-red-600 hover:border-red-600 focus:text-white focus:bg-red-600 focus:border-red-600 focus:ring focus:ring-red-100 active:text-white active:bg-red-600 active:border-red-600 active:ring active:ring-red-100 btn-bulk-delete">
                        <i class="ri-delete-bin-2-line"></i> Bulk Delete
                    </button>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                ...
                <form action="{{ route('roles.bulkDelete') }}" method="POST" id="bulkDeleteForm">
                    @csrf
                    @method('DELETE')
                    <table class="w-full whitespace-nowrap" id="roleTable">
                        <thead class="bg-slate-100">
                            <tr>
                                ...
                                <th class="px-3.5 py-2.5 font-semibold border-b border-slate-200">Role</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="px-3.5 py-2.5 border-y border-slate-200">
                                    <form action="{{ route('roles.destroy', $item->id) }}" method="POST" class="delete-form">
                                        @csrf
                                        @method('DELETE')
...
@push('scripts')
    ...
    <script>
        ...
        function openEditModal(roleId, name) {
            // Set the form action to the correct route with the role ID
            const formAction = `/roles/${roleId}`;
